...
for k in 2**(np.arange(1)+3):
    ...
    for n in 2**(np.arange(1)+18):
        ...
        t0 = time.time()
        mix = mofa.Mofa(d,k,m,PPCA=True)
        imeans = mix.means.copy()
        mix.run_em()
        t = time.time() - t0

        out = open('mofappca_'+str(k)+'_'+str(n)+'_imeans.pkl','wb')
        cPickle.dump(imeans,out)
        out = open('mofappca_'+str(k)+'_'+str(n)+'_fmeans.pkl','wb')
        cPickle.dump(mix.means,out)
        out = open('mofappca_'+str(k)+'_'+str(n)+'_cov.pkl','wb')
        ...
        try:
            ...
            timing = np.atleast_2d(np.loadtxt('mofappcaresults.dat'))
            timing = np.concatenate((timing,res),axis=0)
            np.savetxt('mofappcaresults.dat',timing)
        except:
            res = np.empty((1,3))
            res[0,:] = np.array([k,n,t])
            np.savetxt('mofappcaresults.dat',res)
